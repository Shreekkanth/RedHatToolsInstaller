...
Environment that the dynamic inventory will run against. Options are:

* dev
* test
* prod

**`NOW_SUPPORT_GROUP`** (optional):

Limit the inventory results to specific support group/s. If specifying more than one support group, separate via a colon ':' .

i.e.
```yaml
NOW_SUPPORT_GROUP: 'DCA – AHS – UNIX:DCA - AHS - WINDOWS OPERATIONS'
```

**`NOW_OS`** (optional):

Limit the inventory results to specific a specific Operating System. Options are:

* Linux
* Windows

The results are filtered based on the following strings:

    Linux = CENT|HP-UX|LINUX|RED HAT|RHEL|SOLARIS|SUSE
    Windows = W2K|WINDOWS


### Linux dynamic inventory:

Added Host Variables for the following to append records from CMDB:
    
    Hosted apps
    In Scope for Sox

Ldap queries are executed to group the Linux servers based on the following:
