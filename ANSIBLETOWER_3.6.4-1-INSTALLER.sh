...
echo 'ANSIBLE-TOWER 3.6.4 INSTALLER FOR RHEL 7.x AND RHEL 8.x
...
6. This install was tested with:
        ...
        * Ansible Tower 3.6.4 https://releases.ansible.com/ansible-tower/setup-bundle/ansible-tower-setup-bundle-latest.el7.tar.gz
        ...
        * Ansible Tower 3.6.4 https://releases.ansible.com/ansible-tower/setup-bundle/ansible-tower-setup-bundle-latest.el8.tar.gz
        * Red Hat subscriber channels:
...
then
    ...
    subscription-manager repos --enable --enable rhel-7-server-extras-rpms --enable rhel-7-server-optional-rpms --enable rhel-7-server-supplementary-rpms --enable rhel-server-rhscl-7-rpms --enable rhel-7-server-rpms --enable rhel-7-server-ansible-2.9-rpms
    yum clean all
    rm -rf /var/cache/yum
...
sleep 2
    ...
    yum -q list installed ansible-tower-cli &>/dev/null && echo "ansible-tower-cli is installed" || dnf install -y https://releases.ansible.com/ansible-tower/cli/ansible-tower-cli-el7.repo --skip-broken --noplugins --best --allowerasing
...
echo " "
    ...
    subscription-manager repos --enable ansible-2.8-for-rhel-8-x86_64-rpms --enable rhel-8-for-x86_64-appstream-rpms --enable rhel-8-for-x86_64-baseos-rpms --enable rhel-8-for-x86_64-supplementary-rpms 
    dnf clean all
    rm -rf /var/cache/dnf
echo " "
echo " "
sleep 2
    ...
    yum -q list installed python3-pip-wheel &>/dev/null && echo "wgpython3-pip-wheel is installed" || dnf install -y python3-pip-wheel --skip-broken --noplugins --best --allowerasing
    ...
    yum -q list installed ansible-tower-cli &>/dev/null && echo "ansible-tower-cli is installed" || dnf install -y https://releases.ansible.com/ansible-tower/cli/ansible-tower-cli-el8.repo --skip-broken --noplugins --best --allowerasing
    yum-config-manager --disable epel
echo " "
echo " "
sleep 2
...
then
    ...
    echo '****************************************************************'
        ...
        yum localinstall -y ~/Downloads/ansible-tower-setup-bundle-3.6.4-1/bundle/el7/repos/ansible-tower-dependencies/*.rpm 
...
echo " "
    ...
    echo '****************************************************************'
    echo 'Getting, Expanding, and installing Ansible Tower 3.6.4 for RHEL8'
    echo '****************************************************************'
        mkdir ~/Downloads
        cd ~/Downloads
        ...
        dnf localinstall -y ~/Downloads/ansible-tower-setup-bundle-3.6.4-1/bundle/el8/repos/ansible-tower-dependencies/*.rpm --best --allowerasing
        cd ~/Downloads/ansible-tower*/
        sleep 2
        reset
echo " "
echo " "        
    echo 'What would you like your default Ansible Tower user "admin" password to be?'
        read ADMINPASSWORD
        export $ADMINPASSWORD
        sed -i 's/admin_password='"''"'/admin_password='"'"$ADMINPASSWORD"'"'/g' ~/Downloads/ansible-tower*/inventory 
        sed -i 's/pg_password='"''"'/pg_password='"'"$ADMINPASSWORD"'"'/g' ~/Downloads/ansible-tower*/inventory
        sed -i 's/rabbitmq_password='"''"'/rabbitmq_password='"'"$ADMINPASSWORD"'"'/g' ~/Downloads/ansible-tower*/inventory
echo " "
echo " " 
        sh setup.sh
        sleep 5
