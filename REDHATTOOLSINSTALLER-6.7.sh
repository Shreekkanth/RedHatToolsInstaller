...
echo 'REDHAT TOOLS INSTALLER – FOR RHEL 7.X FOR SETTING UP SINGLE NODE CONFIGURATIONS OF RED HAT MANAGEMENT PORTFOLIO APPLICATIONS FOR P.O.C.'
...
wget https://github.com/ShaddGallegos/RedHatToolsInstaller/blob/master/xdialog-2.3.1-13.el7.centos.x86_64.rpm
...
mkdir RHTI/
touch RHTI/INSTALLERPACKAGES
echo " "
echo " "
}


ls RHTI/INSTALLERPACKAGES
if [ $? -eq 0 ]; then
...
sleep 2
yum install -y screen syslinux python-pip python3-pip rubygems yum-utils vim gcc gcc-c++ git make automake kernel-devel libvirt-client bind dhcp tftp libvirt augeas ruby git --skip-broken
sleep 2
echo " "
echo " "
echo " "
...
touch RHTI/REQUESTSCIENTIFICLINUX
}
...
touch RHTI/ASSOCPLANTOPRODUCTS
}
...
touch RHTI/PARTITION_OS_PXE_TEMPLATE
}
...
touch RHTI/SATREENABLEFOIREWALL
}
...
touch RHTI/DISASSOCIATE_TEMPLATES
}
...
cd ~/Downloads/ansible-tower-setup-bundle-3.6.4-1.el7/
sleep 2
reset
echo " "
echo " " 
...
sed -i 's/admin_password='"''"'/admin_password='"'"'$ADMINPASSWORD'"'"'/g' ~/Downloads/ansible-tower-setup-bundle-3.6.4-1.el7/inventory
sed -i 's/pg_password='"''"'/pg_password='"'"'$ADMINPASSWORD'"'"'/g' ~/Downloads/ansible-tower-setup-bundle-3.6.4-1.el7/inventory
sed -i 's/rabbitmq_password='"''"'/rabbitmq_password='"'"'$ADMINPASSWORD'"'"'/g' ~/Downloads/ansible-tower-setup-bundle-3.6.4-1.el7/inventory
sh setup.sh
sleep 2
echo " "
echo " "
...
echo "Set up Variablesfor Satellite stand by"
VARIABLES1
sleep 2
echo " "
fi
...
ls RHTI/DISASSOCIATE_TEMPLATES
if [ $? -eq 0 ]; then
